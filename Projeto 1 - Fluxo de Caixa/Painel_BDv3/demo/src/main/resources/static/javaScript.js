...
async function findTableLojas(url, tabela, campoData, seletorData) {
  try {
    ...
    data.forEach(loja => {
      ...
      if (loja[campoData]) {
        let dataAtual = parseDataBR(loja[campoData]);
...
async function findTableLojasAdjuste(url, tabela, campoData, seletorData, statusTabela) {
  try {
    tabela.innerHTML = `
      <tr>
        <th>Nome</th>
...
async function findTotalLojas() {
  try {
    const response = await fetch('http://172.16.0.186:7872/totalLojas');
    const data = await response.json();
...
async function findTotalColeta() {
  try {
    const response = await fetch('http://172.16.0.186:7872/totalColeta');
    const data = await response.json();
...
async function findTotalCriticadas() {
  try {
    const response = await fetch('http://172.16.0.186:7872/totalCriticadas');
    const data = await response.json();
...
async function findTotalPendentes() {
  try {
    const response = await fetch('http://172.16.0.186:7872/totalPendentes');
    const data = await response.json();
    document.querySelector('.numberShopsPendente').textContent = data.PENDENTES_TOTAL;
  } catch (error) {
    console.log(error);
  }
}
async function findTotalAtualizadas() {
  try {
    const response = await fetch('http://172.16.0.186:7872/totalAtualizadas');
    const data = await response.json();
    document.querySelector('#numberBD-Atualizadas p').textContent = data.ATUALIZADAS_TOTAL;
  } catch (error) {
    console.log(error);
  }
}
...
async function carregarTotais() {
  const dados = await fetch('http://172.16.0.186:7872/totalAjuste').then(r => r.json());
  ...
  document.querySelector('#numberShopsBoxTotal p.numberShopsTotal').textContent = coleta + pendente + criticada;
}
...
/*   await findTotalColeta();
  ...
  /*         await findTableLojas(
    'http://172.16.0.186:7872/tabela_coleta',
    tableColeta,
    'DATA',
    '#dateColeta'
    );
    await findTableLojas(
      'http://172.16.0.186:7872/tabela_pendentes',
      tablePendentes,
      'DATA',
      '#datePendente'
      );
      await findTableLojas(
        'http://172.16.0.186:7872/tabela_criticadas',
        tableCriticadas,
        'DATA',
        '#dateCriticada'
        ); */
  await findTableLojas(
    'http://172.16.0.186:7872/tabela_atualizadas',
    tableAtualizadas,
    'DATA',
    null
  );
  ...
  await findTableLojasAdjuste(
    'http://172.16.0.186:7872/tabelas_adjuste',
    tableColeta,
    'DATA',
    '#dateColeta',
    'COLETA'
