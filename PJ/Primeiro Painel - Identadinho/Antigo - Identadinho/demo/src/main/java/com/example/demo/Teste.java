...
public class Teste {
    ...
    public ResponseEntity<?> tabela_atualizadas() {
        ...
        SELECT case CTR_FLAG_ATDC
               ...
               ROW_NUMBER() OVER (PARTITION BY TIP_NOME_FANTASIA ORDER BY CTR_DATA_MOV desc) as RN
    ...
    public ResponseEntity<?> totalAjuste() {
        ...
        String sql = """
                WITH ultima_data AS (SELECT CTR_LOJA, MAX(Ctr_data_mov) AS max_data FROM RMS.AG2VCTRL\s
                WHERE Ctr_data_mov BETWEEN ? AND ?  GROUP BY CTR_LOJA), base_recente AS (SELECT a.* FROM RMS.AG2VCTRL a JOIN ultima_data u ON u.CTR_LOJA = a.CTR_LOJA AND u.max_data = a.Ctr_data_mov),\s
                loja_final AS (SELECT b.*,\s
                CASE\s
                
                WHEN CTR_FLAG_ATDC = 'K'
                THEN 'COLETA'
                
                WHEN CTR_FLAG_ATDC = 'P'\s
                THEN 'ATUALIZADA'
                
                WHEN CTR_FLAG_CRIT = 'F' AND CTR_FLAG_ATDC <> 'P'\s
                THEN 'CRITICADA'\s
                
                WHEN CTR_FLAG_ATDC = ' ' AND CTR_FLAG_CRIT = ' ' AND ((CTR_FLAG_INTG = ' ')
                  OR (CTR_FLAG_INTG = 'I' AND CTR_FLAG_QDOC = 'D'))\s
                 AND ((CTR_FLAG_QDOC = 'D') OR (CTR_FLAG_QDOC = ' ' AND CTR_FLAG_INTG = 'I'))\s
                THEN 'PENDENTE'
                
                 END AS STATUS FROM base_recente b)\s
                
                 SELECT STATUS, COUNT(*) AS total FROM loja_final\s
